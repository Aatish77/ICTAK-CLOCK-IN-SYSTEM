...
app.use(Cors())
Mongoose.connect("mongodb+srv://aatish:aatish@cluster0.euclaxo.mongodb.net/clockinsystemdb?retryWrites=true&w=majority", {useNewUrlParser: true})


app.post("/addempapi",async(req,res)=>{
    console.log(req.body)
    const newEmp = new EmpModel({ename :req.body.ename,
    erole :req.body.erole,
    eemail :req.body.eemail,
    ephnnum:req.body.ephnnum,
    epassword :bcrypt.hashSync(req.body.epassword,10)})
await newEmp.save(
    (error,data)=>{
        if(error){
...
app.post("/addadmapi",async(req,res)=>{
    ...
    apassword :bcrypt.hashSync(req.body.apassword,10)})
    await newAdm.save(
    (error,data)=>{
        if(error){
            res.json({"Status":"Error","Error":error})
